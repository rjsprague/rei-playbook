...
export async function POST(req: NextRequest) {
    ...
    const addressLat = JSON.parse(bodyData).addressLatLng.lat;
    const addressLng = JSON.parse(bodyData).addressLatLng.lng;
    ...
    try {
        ...
        if (sendToEmails && sendToEmails.length > 0) {
            for (const email of sendToEmails) {
                ...
                let payload = JSON.stringify({ businessName, businessEmail, subject, body });
        ...
        if (crmWebhookUrl && crmWebhookUrl !== '') {
            ...
            let payload = JSON.stringify({ businessName, name, phone, email, address, zipCode, addressLat, addressLng, utmSource, utmCampaign, utmTerm });
